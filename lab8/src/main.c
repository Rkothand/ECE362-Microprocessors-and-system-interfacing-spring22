...
void bb_write_halfword(int halfword)
{
    ...
    int index =15;
    for(int i=0; i<16; i++){
        int halfword_temp = halfword;
        halfword_temp = halfword_temp>>index;
        halfword_temp &= 1;
        bb_write_bit(halfword_temp);
        index = index - 1;
    }
    /*
    for(int i=0; i<16; i++){
        int halfword_temp = halfword;
        int crosscheck = 0xFFFF;
        crosscheck &= ~index;
        halfword_temp &= ~crosscheck;
        halfword_temp = halfword_temp >> (index-1);
        bb_write_bit(halfword_temp);
        index = index +1;
    }
    */
...
void init_spi1() {
    ...
    SPI1->CR2 &= ~SPI_CR2_DS;
    SPI1->CR2 = SPI_CR2_DS_3 |SPI_CR2_DS_0 |SPI_CR2_NSSP |SPI_CR2_TXDMAEN |SPI_CR2_SSOE;
